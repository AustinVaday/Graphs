...
{
     public:
          Heap(int size = 15);                     			// Parameterized constructor
...
Heap<KeyType, DataType>::Heap(int size)
						: elements(NULL),
						  ...
						  last(0)
{
	// elements will be nullptr if no memory allocated
...
//			delete [] elements;
			...
			{
//				elements[index].setKey(otherHeap.elements[index].getKey());
...
//		cout << "**Inserting data " << item.getData()->getWeight() << "**" << endl;
...
//		cout << "**TOP IS " << elements[root].getData()->getWeight() << "**" << endl;
...
void Heap<KeyType, DataType>::printAll()
{
	if (numElements == 0)
	{
		cout << "Nothing to display, empty heap!";
	}
	else
	{
